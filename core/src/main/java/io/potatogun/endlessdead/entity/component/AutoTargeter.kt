...
class AutoTargeter @JvmOverloads constructor(private val attacker: Entity, override var followRange: Float = 0f, private var targetFinder: Supplier<LivingEntity?>) : Targetable {
	...
	fun isValidTarget(entity: LivingEntity?): Boolean = entity != null && entity.isAlive && !entity.isInvincible && !attacker.isSameTeamWith(entity) && (followRange == 0f || (followRange > 0f && entity.distanceTo(attacker) <= followRange));
}
